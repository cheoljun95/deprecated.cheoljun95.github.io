...
The low performance of the detector module is due to the fact that the types of labels assumed by the GQA are very numerous and complex. Compared to MS COCO, which has frequently been a standard of existing object recognition models, the number of data per category labels is also significantly insufficient. In addition, labels are not exclusive, and similar labels (e.g. pant & trouser) serve as distractors, significantly reducing model performance [12]. Furthermore, the scene graph label of the dataset is only partially labeled and the distribution is also highly biased [7], [8]. This incomplete labeling also resulted in poor performance of NS-VQA (oracle). In [1], the reason why NS-VQA could be successful with respect to CLEVR is also that CLEVR has a very simple label structure.  <br><br>
