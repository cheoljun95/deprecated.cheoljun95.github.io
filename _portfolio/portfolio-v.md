...
<font size="2"> 
   This research project was conducted for the Bachelor’s Thesis of the Department of Computer Science and Engineering at Seoul National University. Here, I tried combining the neural symbolic approach with the scene graph generation technique to tackle the Visual Question Answer task. I mainly focused on the capability of the neural symbolic approach in the GQA dataset, which is close to the real-world data. However, it was not successful, unlike this neural symbolic approach was successful in CLEVR shown by Yi et al. (2018). The performance bottleneck seems to be the inferior performance of scene graph generation. There are details in the article below. 
...
For inferring relations in the form of a graph, the Neural Motif [7] method was introduced. In the method, the relationships of objects are inferred considering the given context. In this study, Transformer [15] was used as a module to calculate the context of the image. Because the number of labels is 200, which is four times higher than [7], but has less data, this study adopted the predicate classification paradigm where the model learns to infer relations (predicates) from the ground truth boxes and labels. <br><br>
...
Neural State Machine (NSM) is an automaton that yields answers with scene graphs and instructions, proposed in [9]. In this method, the activation value is endowed in each node and edge in the scene graph, and each instruction sequentially changes the activation states of the graph to produce the final answer. The instruction activates the node or edge that best matches the instruction. In the NSM, the degree of matching is calculated by the distance in the embedding space, so-called a conceptual embedding. The conceptual embedding also distinguishes whether the construction is applied to nodes or to edges. In this study, the concept of the state machine is introduced, but the activation is determined based on symbol matching without the use of embedding. <br><br>

So, it is necessary to convert the symbolic program label of existing datasets into NSM format. The existing construction consists of "Select," "Relate," "Query," "Exist," "Verify," "Choose," "Choose relation," "And," "Or," "Different," "Common," all of which, except "Select" and "Relate," indicate the requirements and types of questions to produce the final answer. Except for "Different," and "Common," NSM and NS-VQA are equivalent. A new simpler format is defined to make instructions more machine-interpretable ( "instruction = [n,r,o]").
...
The experiment was conducted upon the balanced version of the GQA dataset, which eliminated biases in the distribution of answers. The dataset consists of about 940 K training data and 130 K validation data for questions, and the number of photos corresponding to each division is about 74,000 and 10,000. The model was implemented with PyTorch running on a single NVIDIA Titan X GPU.<br><br>
...
The results of fitting the encoder module are as follows. In contrast to the detector module, the performance is fine enough. This seems to be because the question data is more than the photo data and that the question structure has a relatively simple pattern. The performance degradation factor of the Encoder module is mainly inferring symbol.<br><br>
...
The final results in Table 3 show that NS-VQA's performance is significantly lower than that of previous studies. The most significant reason for the inferior performance is the limited performance of the detector module, as shown in Table 1. Also, the performance difference between NS-VQA and NS-VQA (oracle) in Table 3 supports this perspective.  <br><br>

The low performance of the detector module is because the types of labels assumed by the GQA are very numerous and complex. Compared to MS COCO, which has frequently been a standard of existing object recognition models, the number of data per category label is also significantly insufficient. Also, labels are not exclusive, and similar labels (e.g. pant & trouser) serve as distractors, significantly reducing model performance [12]. Furthermore, the scene graph label of the dataset is only partially labeled and the distribution is also highly biased [7], [8]. This incomplete labeling also resulted in the poor performance of NS-VQA (oracle). In [1], the reason why NS-VQA could be successful with respect to CLEVR is also that CLEVR has a very simple label structure.  <br><br>

The preceding analysis suggests that the NS-VQA system is not scalable for the complex label structure. Embedding can be introduced as a solution to mitigate this problem, by calculating whether or not the symbol matches the embedding space ([9],[11]). In addition, the application of end-to-end or joint training methods is indispensable to introduce such embedding. In this study, the fundamental modifications of the system are needed to apply the end-to-end training because it is designed for the purpose of reviewing the performance limits when each part of the system is learned in the absence of an end-to-end.

 In [1], [9], the reinforcement learning method was used for joint training. and in [11], the encoder module and executer module are trained end-to-end. <br><br>
